...
```{r}
sim_omega <- simrandom1$omega
sim_sigma <- simrandom1$sigma
Y <- simrandom1$data
```
...
tic()
fit <- ghs_like_mcmc(S, N, burnin = 500, nmc = 1000, thin = 1,
                     seed = 1, save_omega = TRUE, verbose = TRUE)
toc()
```
...
horseshoe_credible_graph <- edges_from_ci(horseshoe_model$OmegaSamples)
...
graph_evaluation(true_graph, horseshoe_credible_graph)
```
