concentration_horseshoe <- function(Y, burn = 1000, iter = 5000, lambda = 1, 
  ...
  XiSamples <- rep(0, iter)
  EtaSamples <- array(0, dim = c(p, p, iter))
  ActiveMatrixSamples <- array(0, dim = c(p, p, iter))
  ...
  for (i in 1:(iter + burn)) {
    ...
    for (j in 1:p) {
        ...
        if (auto.threshold == TRUE) {
          if (i %% t == 0) {
            if (u_i < p_i) {
              ...
              # 방법 1
              m_eff[j] <- sum(1/(sqrt((S[j, j]+lambda)^(-1) * v12[-j] * 
                                        diag(estimate_Omega[-j, -j])) + 1))
              
              # 방법 2 : Ksheer sagar version
              #m_eff[j] <- sum(1/(1+v12[-j]/(estimate_Omega[j, j]*diag(S[-j, -j]))))
            }
          }
      ...
      if(active_num != 0) {
        ...
        C <- (S[j, j] + lambda) * sweep(sqrt_D[k] * inv_omega11[h, h, drop = FALSE], 2, 
                                        sqrt_D[k], "*") + diag(active_num)
        L <- chol(C)
        f_star <- t(L) %*% f[h]
        x <- sqrt_D[k] * S[k, j] + f_star
        u <- -sqrt_D[k] * solve(C, x)
        
        #x1 <- sqrt_D[k] * S[k, j] + f_star
        #u <- - sqrt_D[k] * solve(C, x1)
        #x2 <- solve(t(L), x1)
        #u <- - sqrt_D[k] * solve(L, x2)
        
        #x1 <- sqrt_D[k] * S[k, j] - f_star[h]
        #x2 <- solve(t(L[h, h]), x1)
        #u <- - sqrt_D[k] * solve(L[h, h], x2)
        # ------------------------------------------------
    ...
    if (i > burn) {
      ...
      EtaSamples[, , i - burn] <- eta
      XiSamples[i-burn] <- xi
      ActiveMatrixSamples[, , i - burn] <- active_matrix
...
# meff 계산을 exact algorithm에서도 진행하도록 추가한 함수 
concentration_horseshoe2 <- function(Y, burn = 1000, iter = 5000, lambda = 1, 
                                     approximate = FALSE, fixed_threshold = 0, 
                                     auto.threshold = FALSE, t = 10, 
                                     adapt_p0 = 0, adapt_p1 = -4.6*10^(-4)) {
  N <- nrow(Y)
  ...
  for (i in 1:(iter + burn)) {
    ...
    for (j in 1:p) {
      ...
      if (approximate == TRUE) {
        ...
        # exact algorithm에서도 m_eff 계산하여, 그래프 추정
        if (i %% t == 0) {
          if (u_i < p_i) {
